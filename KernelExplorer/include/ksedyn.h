...
#include <ksebase.h>
#define KSE_DYN_CONFIGURATION_VERSION           ((ULONG)16)
#define KSE_DYN_SESSION_TOKEN_PUBLIC_KEY_LENGTH ((ULONG)539)
#define KSE_DYN_CLASS_NTOSKRNL                  ((USHORT)0)
#define KSE_DYN_CLASS_NTKRLA57                  ((USHORT)1)
#define KSE_DYN_CLASS_LXCORE                    ((USHORT)2)

#include <pshpack1.h>


typedef struct _KSE_DYN_KERNEL_FIELDS
{
...
} KSE_DYN_KERNEL_FIELDS, * PKSE_DYN_KERNEL_FIELDS;

typedef KSE_DYN_KERNEL_FIELDS KSE_DYN_NTOSKRNL_FIELDS;
typedef PKSE_DYN_KERNEL_FIELDS PKSE_DYN_NTOSKRNL_FIELDS;
typedef KSE_DYN_KERNEL_FIELDS KSE_DYN_NTKRLA57_FIELDS;
typedef PKSE_DYN_KERNEL_FIELDS PKSE_DYN_NTKRLA57_FIELDS;

typedef struct _KSE_DYN_LXCORE_FIELDS   
{
...
} KSE_DYN_LXCORE_FIELDS, * PKSE_DYN_LXCORE_FIELDS;

typedef struct _KSE_DYN_FIELDS
{
    ULONG FieldsId;
    USHORT Length;
    BYTE Fields[ANYSIZE_ARRAY];
} KSE_DYN_FIELDS, * PKSE_DYN_FIELDS;

typedef struct _KSE_DYN_DATA
{
    USHORT Class;
    USHORT Machine;
    ULONG TimeDateStamp;
    ULONG SizeOfImage;
    ULONG Offset;
} KSE_DYN_DATA, * PKSE_DYN_DATA;

typedef struct _KSE_DYN_CONFIG
{
    ULONG Version;
    BYTE SessionTokenPublicKey[KSE_DYN_SESSION_TOKEN_PUBLIC_KEY_LENGTH];
    ULONG Count;
    KSE_DYN_DATA Data[ANYSIZE_ARRAY];
    // BYTE Fields[ANYSIZE_ARRAY];
} KSE_DYN_CONFIG, * PKSE_DYN_CONFIG;

#include <poppack.h>

#ifdef _WIN64
extern CONST BYTE KseDynConfig[];
extern CONST ULONG KseDynConfigLength;
#endif
