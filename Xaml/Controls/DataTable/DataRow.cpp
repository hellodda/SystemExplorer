#include "pch.h"
#include "DataRow.h"
#if __has_include("Controls/DataRow.g.cpp")
#include "Controls/DataRow.g.cpp"
#endif
...
namespace winrt::SystemExplorer::Xaml::Controls::implementation
{
    ...
    Panel DataRow::InitializeParentHeaderConnection()
    {
        ...
        if (auto itemsPresenter = winrt::SystemExplorer::Xaml::DependencyObjectEx::FindAscendant<ItemsPresenter>(*this))
        {
            ...
            else
            {
                ...
                panel = winrt::SystemExplorer::Xaml::DependencyObjectEx::FindDescendant<Panel>(itemsPresenter,
                    [](auto&& element) { return element.template try_as<Grid>() || element.template try_as<winrt::SystemExplorer::Xaml::Controls::DataTable>(); });
            }

            // Check if we're in a TreeView
            _isTreeView = winrt::SystemExplorer::Xaml::DependencyObjectEx::FindAscendant<TreeView>(itemsPresenter).try_as<TreeView>() != nullptr;
        }

        // 1b. If we can't find the ItemsPresenter, then we reach up outside to find the next thing we could use as a parent
        if (panel == nullptr) panel = winrt::SystemExplorer::Xaml::DependencyObjectEx::FindAscendant<Panel>(*this, [](auto&& element) { return element.template try_as<Grid>() || element.template try_as<DataTable>(); });

        // Cache actual datatable reference
        if (auto table = panel.try_as<winrt::SystemExplorer::Xaml::Controls::DataTable>())
        {
            _parentTable = table;
        ...
        if (childCount > 0)
        {
                ...
                && _parentTable.Children().Size() == childCount)
            {
                ...
                for (uint32_t i = 0; i < childCount; i++)
                {
                    ...
                    auto dataColumn = _parentTable.Children().GetAt(i).try_as<winrt::SystemExplorer::Xaml::Controls::DataColumn>();

                    if (dataColumn == nullptr) continue;

                    auto colImpl = winrt::get_self<winrt::SystemExplorer::Xaml::Controls::implementation::DataColumn>(dataColumn);
                    ...
                    if (colImpl->CurrentWidth().GridUnitType == GridUnitType::Auto)
                    {
                        ...
                        if (i == 0 && _isTreeView)
                        {
                            ...
                            auto parentContainer = winrt::SystemExplorer::Xaml::DependencyObjectEx::FindAscendant(*this, L"MultiSelectGrid").try_as<Grid>();
    ...
    Size DataRow::ArrangeOverride(Size finalSize)
    {
        ...
        if (_parentPanel != nullptr)
        {
            ...
            for (const UIElement& child : elements)
            {
                ...
                {
                    if (auto table = _parentPanel.try_as<winrt::SystemExplorer::Xaml::Controls::DataTable>(); table && column < table.Children().Size()) {
                        // TODO: This is messy...
                        auto col = table.Children().GetAt(column++).try_as<winrt::SystemExplorer::Xaml::Controls::DataColumn>();
